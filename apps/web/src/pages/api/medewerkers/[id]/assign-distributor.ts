...
/**
 * POST /api/medewerkers/[id]/assign-distributor
 * Body: { distributor_id, role_level: 'staff' | 'manager' }
 */
export const POST: APIRoute = async ({ request, params, cookies }) => {
  ...
  let body: { distributor_id?: string; role_level?: string };
  try { body = await request.json(); } catch {
    return new Response(JSON.stringify({ error: "Ongeldige JSON" }), { status: 400 });
  }

  const { distributor_id, role_level = "staff" } = body;
  ...
  const { error } = await auth.supabase
    ...
    .insert({ user_id, distributor_id, role_level });

  if (error) {
    const msg = error.code === "23505" ? "Medewerker is al gekoppeld aan deze distributeur." : error.message;
